...
<script type="text/javascript">
	...
	function sendRequestPartPosition()//查分项一 
	{
		...
		//alert(partSelectObj.options[partSelectObj.selectedIndex].value);
	...
	function listPartPosition() {
		...
		opobj.length = 1;
		//var Fsobj = document.getElementById("FsId"); //删除Fsobj选项
		//Fsobj.length = 1;
	...
	function sendRequestAccQuantity() {
		...
		//var fsId = document.getElementById("FsId").options[fsSelect].value;
		var url = "countAccQuantity";
		//找出工程名称
		var flag = window.opener.document.addmissionform.prId.selectedIndex;
		var gcmc = window.opener.document.addmissionform.prId.options[flag].value;
		//找出分部
		var partSelect = document.getElementById("partId").selectedIndex;
		var gcbw = document.getElementById("partId").options[partSelect].value;
		//找出所属部门
		//flag = window.opener.document.addmissionform.dpCode.selectedIndex;
		//var ssbm = window.opener.document.addmissionform.dpCode.options[flag].value;
		//找出结算单位
		flag = window.opener.document.addmissionform.cuId.selectedIndex;
		var jsdw = window.opener.document.addmissionform.cuId.options[flag].value;
		//找出工程部位 
		...
		//找出栋号
		flag = window.opener.document.getElementsByName("unitnumber")[0].selectedIndex;
		var unumber = window.opener.document.getElementsByName("unitnumber")[0].options[flag].value;
		//找出任务书类型
		var mtid = window.opener.document.getElementsByName("mtid")[0].value;
		if (gcmc == null || gcmc == "" || gcbw == null || gcbw == ""
						|| jsdw == null
						|| jsdw == "" || gzxm == null || gzxm == ""
						|| dw == null || dw == "" || unumber == null
						|| unumber == "" || psid == null || psid == "" || mtid==null || mtid=="") {
			//alert(gcmc+gcbw+ssbm+jsdw+psid+gzxm+dw+unumber);
			//alert("gcmc=" +gcmc + "gcbw=" + gcbw + "ssbm=" + ssbm + "jsdw=" + jsdw + "gzxm=" + gzxm + "dw=" + dw + "unumber=" + unumber + "psid=" + psid);
			return;
		} else {
	...
	function submit() {
		...
		if (mtCode != '7') {
			...
			window.opener.document.getElementsByName("unitId")[index].value = unitId;
			...
			if (mtCode == '1') {
				...
				var ykl = document.getElementById("ykl").value;
				window.opener.document.getElementsByName("ykl")[index].value = ykl;
				var jjykl = document.getElementById("jjykl").value;
				window.opener.document.getElementsByName("jjykl")[index].value = jjykl;
			}
		} else {
			var wageSum = document.getElementById("wageSum").value;
			var accQuantity = document.getElementById("accQuantity").value;
		...
		if(mtCode == '4'){
			var getDisplay = document.getElementById("innerIdselect").style.display;
			if (getDisplay == 'none') {	// select没显示，就用input的值
		...
		window.opener.document.getElementsByName("partId")[index].value = partId;
		window.opener.document.getElementsByName("psId")[index].value = psId;
		window.opener.document.getElementsByName("operationId")[index].value = operationId;
		//window.opener.document.getElementsByName("fsId")[index].value = fsId;

		window.opener.document.getElementsByName("partPositionText")[index].value = partName;
		window.opener.document.getElementsByName("partItemoneText")[index].value = psName;
		window.opener.document.getElementsByName("partItemtwoText")[index].value = operationName;
	...
	function cancel() {
		//6var table = window.opener.document.getElementById("customers");
...
<body>
	<table cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
		<tbody>
			<tr>
				...
				<font style="font-size:1.2em;color: white">新建任务书</font>
				</td>
			</tr>
		</tbody>
	</table>
	...
	<input type="hidden" value="${parts }" id="parts" />
...
<c:choose>
	...
	<table id="preaddMission">
		...
		<tr>
			...
			<td>
				<div>
					<input type="text" value="" id="price" readonly />
				</div>
			</td>
		</tr>
		</c:when>
		...
		<tr>
			...
			<td>
				<input type="text" value="" id="realQuantity" />
			</td>
		</tr>
		<c:choose>
		<c:when test="${ mtCode != 1}">
			<input type="hidden" value="" id="accQuantity" />
		</c:when>
		<c:otherwise>
		<tr>
			...
			<td>
				<div>
					<input type="text" value="" id="accQuantity" readonly />
				</div>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				<div>
					<textarea rows="4" cols="50" name="floor" id="floor"></textarea>
				</div>
			</td>
		</tr>
		<tr>
	...
	<table id="preaddMission">
		...
		<tr>
			...
			<td>
				<div>
					<input type="text" value="" id="accQuantity" />
				</div>
			</td>
		</tr>
		<tr>
			...
			<td>
				<div>
					<input type="text" value="" id="wageSum" />
				</div>
			</td>
		</tr>
		<tr>
	...
	<c:if test="${error != null }"><h3 style="color:red" align="center">${error }</h3></c:if>
