...
var rowcount;	
	...
	function editMission(obj)
	{
		...
		window.open("<%=path%>/preEditMission?index=" + index + "&prId=" + prId + "&prName=" + prName + "&pcpId=" + pcpId + 
	...
	<table cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
		<tbody>
			<tr>
				...
				<font style="font-size:1.2em;">修改计时任务书</font>
				</td>
			</tr>
		</tbody>
	</table>
		...
		<input type="hidden" name="mId" value="${mission.mId}" id="mId"/> 
			...
			<table id="viewMission">
				...
				<tr>
					<td colspan="12">
						<div align="center">计 时 任 务 书 详 细 信 息</div>
						<div align="right">
      ...
      <c:forEach items="${missionContentList}" var="mcl">
        <tr>
            <td>
            <input type="text" name="mcCode" readonly value="${mcl.mcCode }"/>
            </td>
            <td>
            <input type="hidden" value="${mcl.pId}" name="pId" id="pId">
            <input type="text" name="pName" readonly value="${mcl.pName }"/>
            </td>
            <td>
            <input type="hidden" value="${mcl.psId}" name="psId" id="psId">
            <input type="text" name="psName" readonly value="${mcl.psName }"/>
            </td> 
            <td>
            <input type="hidden" value="${mcl.oId}" name="oId" id="oId">
            <input type="text" name="oName" readonly value="${mcl.oName }"/>
            </td> 
            <td>
            <input type="text" name="floor" value="${mcl.floor }"/>
            </td>
             <td>
             <input type="hidden" value="${mcl.unId}" name="unId" id="unId">
            <input type="text" name="unName" readonly value="${mcl.unName }"/>
            </td> 
             <td>
            <input type="text" name="price" value="${mcl.price }"/>
            </td> 
             <td>
            <input type="text" name="realQuantity" value="${mcl.realQuantity }"/>
            </td> 
             <td>
            <input type="text" name="wageSum" readonly value="${mcl.wageSum }"/>
            </td> 
             <td>
            <input type="text" name="remark" value="${mcl.remark }"/>
            </td>
            <td>
            ...
            <a href="javascript:void(0)" onclick='delCurrentRow(this)'>删除</a>
					...
					<tr>
						<td colspan="12">
							...
							<input type="hidden" value="${auditInfoList}" name="auditComment" id="auditComment">
							</div>
						</td>
