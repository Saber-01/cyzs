...
<script type="text/javascript">
	...
	function newAddMissionTree(otable)
	{
		...
		var prName=prNameSelect.options[prNameSelect.selectedIndex].value;//2010-11-29 meng 工程名称有#号的问题 改用主键
		var cuName=cuNameSelect.options[cuNameSelect.selectedIndex].value;//2011-2-28
	
		//2010-11-29 meng 栋号有#号的问题 改用主键
	...
	function addNewContractRow(otable) {
		...
		rowcount = document.getElementById("customers").rows.length - 1;
	...
	function delRow(obj)	// 这是已存在的missionContent的删除方法
	{
	...
	<form name="addmissionform" method="post" action="addJJMission" onSubmit="">
		...
		<table id="addmission" class="table table-striped table-bordered table-condensed">
			...
			<tr>
				...
				<td colspan=2>&nbsp;&nbsp;<a onclick="window.open('preUploadFile?mid=${user.userLoginName }',
				'添加附件','width=800,height=650,toolbar=no,scrollbars=anto,menubar=no,top=50,left=200')">添加附件</a></td>
			</tr>
		</table>
		<table id="customers">
			<tr>
		...
		<tr>
			<td><div align="center"><input type="text" size="5" name="contentSerial" value="<%= i= i + 1 %>" 
						readonly onBlur=checkContentSerial(this);></div></td>
			<td><div align="center"><input type="text" size="8" name="partPositionText" value="${mc.pName }" readonly> 
				<input type="hidden" value="${mc.pId }" name="partId"></div></td>
			<td><div align="center"><input type="text" size="8" name="partItemoneText" value="${mc.psName }" readonly> 
				<input type="hidden" value="${mc.psId }" name="psId"></div></td>
			<td><div align="center"><input type="text" size="8" name="partItemtwoText" value="${mc.oName }" readonly> 
				<input type="hidden" value="${mc.oId }" name="operationId"></div></td>
			<td><div align="center"><input type="text" size="8" name="floor" value="${mc.oName }"></div></td>
			<td><div align="center"><input type="text" size="7" name="unName" value="${mc.unName }" readonly> 
				<input type="hidden" value="${mc.unId }" name="unitId"></div></td>
			<td><div align="center"><input type="text" size="7" name="price" value="${mc.price }" readonly></div></td>
			<td><div align="center"><input type="text" size="8" name="realQuantity" value="${mc.realQuantity }"></div></td>
			<td><div align="center"><input type="text" size="8" name="wageSum" value="${mc.wageSum }" readonly></div></td>
			<td><div align="center"><input type="text" size="8" name="accQuantity" value="${mc.accumulateSum != null ? mc.accumulateSum : 0 }" readonly></div></td>
			<td><div align="center"><input type="text" size="8" name="jjykl" value="${mc.jjykl }" readonly> 
				<input type="hidden" size="8" name="ykl" value="${mc.ykl }" readonly></div></td>
			<td><div align="center"><input type="text" size="15" name="remark" value="${mc.remark }"></div></td>
			<td><div align="center">
				<a id="FlowListLabel" style="color: blue; cursor: hand;text-decoration:underline" onclick="delRow(this)">
					删除
				</a></div>
			</td>
		</tr>
		...
		<div id="clickbutton" style="display:none;">
			...
			<input onclick="window.history.go(-1);" type="button" value="取消" class="easyui-linkbutton" iconCls="icon-ok">
		</div>
		</c:if>
		<br>
