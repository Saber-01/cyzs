...
<body>
    <div class="container" style="height:100%">
       <table id="dg" style="width:100%;height:99%" title="计时单价管理" data-options="
                rownumbers:true,
                singleSelect:false,
                autoRowHeight:false,
                pagination:true,
...
function edit(id){
	   var _url = "${pageContext.request.contextPath }/timePrice/toEditTimePrice/" + id;
	   $('<div></div>').dialog({
	                    id : 'newDialog',
	                    title : '修改计时合同单价',
	                    width : 602,
...
function reloadtb(){
  $("#dg").datagrid("reload",{                          
   ...
   function add(){
   var _url = "${pageContext.request.contextPath }/timePrice/toAddTimePrice";
   $('<div></div>').dialog({
                    id : 'newDialog',
                    title : '添加计时合同单价',
                    width : 602,
...
{  
    ...
    $.ajax({  
                contentType: 'application/json', 
                type:"POST",  
                url:"${pageContext.request.contextPath }/contract/changeJob",
                dataType:"json", 
                data:postdata,
                success:function(data){
                 $("#jobKey").empty(); 
...
{  
    ...
    $.ajax({  
                contentType: 'application/json', 
                type:"POST",  
                url:"${pageContext.request.contextPath }/contract/changeUnitName",
                dataType:"json", 
                data:postdata,
                success:function(data){
                 $("#unId").empty();                
